...
const EP={木:'#4A7A38',火:'#C03828',土:'#8C6840',金:'#787E7A',水:'#304E8A'};

const traitData={
  木:{title:'翠綠生命力',trait:'充滿生長能量，有規劃力與仁慈心，但有時難以放手過去',fate:'命運中常有新的萌芽與開始，適合創業與教育領域深耕',color:'翠綠 · 橄欖 · 嫩芽'},
...
const ziweiMap=[
  ...
  {name:'太陽星',glyph:'陽',trait:'你天生是給予者，像太陽一樣溫暖周圍的人，但有時忘了照亮自己。你的存在對家人與夥伴是一種穩定的光源。'},
  ...
  {name:'天梁星',glyph:'梁',trait:'你是老靈魂，天生帶著一種慈悲的長者氣度。你常常是身邊人的精神支柱，守護著那些比你脆弱的人。'},
...
function dp(y, m, d, hIdx) {
  let calcY = y, calcM = m, calcD = d;
  if (hIdx === 23) {
    const nextDay = new Date(y, m - 1, d + 1);
    calcY = nextDay.getFullYear();
    calcM = nextDay.getMonth() + 1;
    calcD = nextDay.getDate();
  }
  
  const jd = getJulianDay(calcY, calcM, calcD);
  const gzIndex = Math.floor(jd + 0.5 + 49) % 60;
  const ti = (gzIndex % 10 + 10) % 10;
  const di = (gzIndex % 12 + 12) % 12;
  return { tg: TG[ti], dz: DZ[di], tE: TGE[ti], dE: DZE[di], ti, di };
}

function ypAndMp(y, m, d) {
  // 公曆 1~12 月各自對應的「節」（交月節氣）平均交節公曆日期
  // [0]1月小寒, [1]2月立春, [2]3月驚蟄, [3]4月清明, [4]5月立夏, [5]6月芒種, 
  // [6]7月小暑, [7]8月立秋, [8]9月白露, [9]10月寒露, [10]11月立冬, [11]12月大雪
  const termDays = [5, 4, 5, 4, 5, 5, 7, 7, 7, 8, 7, 7];

  let baziYear = y;
  
  // 2月立春前算上一年的年柱（立春一般落在 2月4日前後）
  if (m < 2 || (m === 2 && d < termDays[1])) {
    baziYear = y - 1;
  }
  
  // 計算干支年柱索引 (1900年為庚子年，(1900-4)%60 = 36)
  const yearGzIndex = (baziYear - 4) % 60;
  const yTi = (yearGzIndex % 10 + 10) % 10;
  const yDi = (yearGzIndex % 12 + 12) % 12;

  // 計算八字月分索引（以寅月為 0）
  // 如果當月日期小於該月交節日，代表還屬於上一個月節氣
  let solarMonthIdx;
  if (d < termDays[m - 1]) {
    solarMonthIdx = (m - 2 + 12) % 12;
  } else {
    solarMonthIdx = m - 1;
  }

  // 節氣月轉八字地支月：
  // 1月小寒後為丑月(1)，2月立春後為寅月(2)，3月驚蟄後為卯月(3)，4月清明後為辰月(4)...
  // 因此當前節氣月地支索引為 (solarMonthIdx + 1) % 12
  const mDi = (solarMonthIdx + 1) % 12;

  // 八字月數（以正/寅月為 0 計算五虎遁）
  // 寅月(地支2)對應 offset=0，卯月(地支3)對應 offset=1...
  const monthOffset = (mDi - 2 + 12) % 12;

  // 五虎遁月干推算：甲己之年丙作首，乙庚之歲戊為頭...
  const mTi = ((yTi % 5) * 2 + 2 + monthOffset) % 10;

  return {
    Y: { tg: TG[yTi], dz: DZ[yDi], tE: TGE[yTi], dE: DZE[yDi] },
    M: { tg: TG[mTi], dz: DZ[mDi], tE: TGE[mTi], dE: DZE[mDi] }
  };
}

function hp(dayTgIndex, hSelectIndex) {
  const zhiIndex = Math.floor((hSelectIndex + 1) / 2) % 12;
  const stemIndex = ((dayTgIndex % 5) * 2 + zhiIndex) % 10;
  return { tg: TG[stemIndex], dz: DZ[zhiIndex], tE: TGE[stemIndex], dE: DZE[zhiIndex] };
}
...
function compute(){
  const y = +document.getElementById('iY').value,
        ...
        hIdx = +document.getElementById('iH').value;

  const { Y, M } = ypAndMp(y, m, d);
  const D = dp(y, m, d, hIdx);
  const H = hp(TG.indexOf(D.tg), hIdx);
...
function renderBaziPanel(){
  ...
  document.getElementById('healAlert').innerHTML=`
    <div style="margin-top:0.875rem;padding:0.875rem;background:${EB[S.mis]};border-radius:8px;border-left:2px solid ${EC[S.mis]}">
      <div style="font-size:1rem;font-weight:500;color:${ET[S.mis]};margin-bottom:3px">療癒核心：補${S.mis}能量</div>
      <div style="font-size:1rem;color:${ET[S.mis]};line-height:1.6">${S.dom}為主色（強旺主結構），${S.mis}為療癒色（補充張力），這幅畫本身就是你的平衡處方</div>
    </div>`;
  ...
  document.getElementById('palStrip').innerHTML=segs.map(([e,r,pct])=>`
    <div class="pal-seg" style="background:${EP[e]};flex:${parseInt(pct)};color:${e==='金'?'#5A5852':'rgba(255,255,255,.9)'}">
      <span>${e}</span><span style="font-size:13px;opacity:.75">${r} ${pct}</span>
    </div>`).join('');
...
function renderS4(){
  ...
  const moodContent={
    healed:{
      ...
      sub:`命盤中${dom}的精準與自律，讓你為每一件事設下嚴格標準——但你忘了，有時候，讓心軟下來也是一種力量。這幅畫補入了${mis}的能量，不是要改變你，而是讓你找回被壓進去的那部分。`,
  ...
  document.getElementById('healBox').innerHTML=`
    ...
    <div class="heal-affirmation">✦ 療癒語句：「${mc.affirmation}」</div>`;

  const sorted=Object.entries(S.elems).sort((a,b)=>b[1]-a[1]);
  document.getElementById('outPal').innerHTML=[
    [dom,'主色·50%','50'],[mis,'療癒·30%','30'],[sorted[1][0],'輔色·20%','20']
  ].map(([e,r,f])=>`
  ...
  document.getElementById('promptNote').textContent=`${dom}（主）× ${mis}（療癒）· 記憶：${S.mem||'—'} · 質地：${S.textures.map(t=>t.split(' ')[0]).join('+')||'—'}`;
  ...
  document.getElementById('finalCN').innerHTML=`
    畫面以「<strong>${dom}</strong>」的色彩與幾何為主結構（約60%），對應${traitData[dom].title}的性格底色。以「<strong>${mis}</strong>」的療癒色系作為張力補充（約30%），帶入${traitData[mis].title}能量，形成命盤最核心的療癒對話。${symCN}
    <br><br>
...
function applyAuthState() {
  ...
  if (isAdmin) {
    ...
    if(dot) { dot.style.background = '#6DB85A'; dot.style.boxShadow = '0 0 8px #6DB85A'; }
    const lbl = document.getElementById('authLabel');
    if(lbl) { lbl.textContent = '主用戶模式 · 完整內容已解鎖'; lbl.style.color = '#8DD870'; }
    const btn = document.getElementById('authToggleBtn');
    if(btn) {
      btn.textContent = '已解鎖';
      btn.style.borderColor = '#6DB85A';
      btn.style.color = '#8DD870';
      btn.onclick = null;
    }
  } else {
    cards.forEach(id => document.getElementById(id)?.classList.remove('unlocked'));
    const dot = document.getElementById('authDot');
    if(dot) { dot.style.background = 'var(--text3)'; dot.style.boxShadow = '0 0 6px var(--text3)'; }
    const lbl = document.getElementById('authLabel');
    if(lbl) { lbl.textContent = '遊客模式 · 部分內容已鎖定'; lbl.style.color = 'var(--text3)'; }
    const btn = document.getElementById('authToggleBtn');
    if(btn) {
      btn.textContent = '主用戶登入';
...
.logo{font-family:'DM Mono',monospace;font-size:14px;letter-spacing:5px;color:#444;text-transform:uppercase;margin-bottom:6px;}
.title{font-size:22px;font-weight:300;letter-spacing:6px;color:#e8eaf0;margin-bottom:2px;}
.subtitle{font-size:14px;font-family:'DM Mono',monospace;color:#444;letter-spacing:3px;margin-bottom:36px;}
.divider{height:0.5px;background:linear-gradient(90deg,transparent,#2b4f8c,#6c8eff,transparent);margin:24px 0;}
.section-title{font-family:'DM Mono',monospace;font-size:14px;letter-spacing:3px;color:#6c8eff;text-transform:uppercase;margin-bottom:12px;}
.tag-row{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:20px;}
.tag{padding:4px 12px;border-radius:20px;border:0.5px solid #2b4f8c;font-size:14px;color:#a0b4ff;font-family:'DM Mono',monospace;letter-spacing:1px;}
.pal{display:flex;border-radius:8px;overflow:hidden;height:36px;margin-bottom:6px;}
.color-note{font-size:14px;color:#555;font-family:'DM Mono',monospace;margin-bottom:20px;}
.star-row{display:flex;align-items:center;gap:12px;margin-bottom:16px;}
.star-glyph{width:40px;height:40px;border-radius:50%;background:#1a2240;border:1px solid rgba(43,79,140,0.25);display:flex;align-items:center;justify-content:center;font-size:17px;color:#6c8eff;flex-shrink:0;}
.star-name{font-size:14px;color:#e8eaf0;letter-spacing:2px;}
.star-trait{font-size:14px;color:#888;line-height:1.8;margin-bottom:20px;}
.heal-main{font-size:14px;color:#e8eaf0;letter-spacing:1px;line-height:1.8;margin-bottom:10px;}
.heal-sub{font-size:14px;color:#aaa;line-height:2;margin-bottom:14px;}
.heal-aff{font-size:14px;color:#33ffaa;letter-spacing:1px;line-height:1.8;padding:10px 14px;border-left:2px solid rgba(51,255,170,0.25);background:rgba(51,255,170,0.04);}
.booking-btn{margin:30px auto;display:block;text-align:center;}
.booking-btn a{
display:inline-block;padding:13px 36px;
border:0.5px solid #6c8eff;border-radius:8px;
background:rgba(108,142,255,0.1);
color:#6c8eff;font-family:'Noto Serif TC',serif;
font-size:14px;letter-spacing:3px;
text-decoration:none;}
.booking-btn a:hover{border:0.5px solid #33ffaa;color:#33ffaa;font-size:18px;}
.footer{margin-top:40px;padding-top:16px;border-top:0.5px solid #111;text-align:center;font-family:'DM Mono',monospace;font-size:12px;color:#333;letter-spacing:3px;}
...
<br>這只是您宇宙參數的10%顯影。完整的LIFE+Cube將結合您的生命座標與生命故事看板，生成專屬的加密數位方塊。想開啟您的生命方塊嗎？<br></div>
<div class="booking-btn">
  <a href="${window.location.href.split('?')[0]}#booking">
    預約訂製 · Book Custom Service
  </a>
</div>
